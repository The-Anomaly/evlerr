                ...
                {properties.map((item) => (
                    <div key={item._id}>
                        <PropertyCards
                            ...
                            detailsTitle={'Apartment'} onClick={() => selectResourceType(item)} agentImage={item.agentId.profilePicture ? item.agentId.profilePicture.url : avatar} agentName={item.agentId.username} years={item.createdAt}
